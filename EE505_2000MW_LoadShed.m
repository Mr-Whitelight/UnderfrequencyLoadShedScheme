...
Aoffset_x = 1.0; % Offset for x-coordinate
...
% Display the marker coordinates with an offset
Boffset_x = 0.4; % Offset for x-coordinate
...
text(t2 + Boffset_x, UFLSblock2nd + Boffset_y, ...
    ...
    'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'left');
% Display the marker coordinates with an offset
Coffset_x = 0.8; % Offset for x-coordinate
Coffset_y = 0.02; % Offset for y-coordinate
text(t3 + Coffset_x, UFLSblock3rd + Coffset_y, ...
    sprintf('t3: %.4f s, Target frequency: %.1f Hz', t3, UFLSblock3rd), ...
    'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'left');
% Display the marker coordinates with an offset
Doffset_x = 20.0; % Offset for x-coordinate
Doffset_y = 0.8; % Offset for y-coordinate
text(t3 + Doffset_x, UFLSblock3rd + Doffset_y, ...
    sprintf('Proposed 2000MW Load Shed Settings: \n2.5%% @ 49.0 Hz,\n5.8333%% @ 48.8 Hz and\n8.333%% @ 48.6 Hz'), ...
    'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'left');
